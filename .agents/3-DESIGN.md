...
| Pink | `neo-pink` | `#FBCFE8` | Badge teknis/meta (cadangan — 0 pemakaian di src/ per 2026-09-04, badge "BCRYPT HASH" login dihapus) |
...
Warna flat per konteks (dashboard=Yellow, pembayaran=Green, pengeluaran=Coral, anggota=Purple, laporan=Orange, status anggota=Sky), `border-b-[2.5px] border-black`, judul uppercase font-extrabold. (Amendemen 2026-09-02: /pembayaran split ke tab sendiri diberi Green — semua warna lain sudah terpakai; /anggota Purple menggantikan Sky sesuai keputusan user FASE-3.) **Amendemen 2026-09-04 (user-directed):** badge periode di header /laporan — menampilkan **bulan/tahun TERPILIH FilterBar** (fix 2026-09-04 sesi lanjutan; sebelumnya bulan berjalan — kontradiksi dengan file unduhan saat filter diganti) — `rounded border border-black bg-neo-sky px-1.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wider`, sebelum subtitle. Posisi periode per halaman: dashboard = subtitle "Periode: <bulan> <tahun>", pengeluaran = subtitle, laporan = badge sky, pembayaran = tanpa periode (badge "1-Tap" saja), anggota = tanpa periode di header (periode + badge "BULAN INI" di konten dengan chevron).

**Tombol WhatsApp reminder (amendemen 2026-09-04, user-directed + fix sesi lanjutan):** tombol "WA" kini **hanya di /anggota** (row anggota): `<a target="_blank" rel="noopener noreferrer">` wa.me, **SIBLING tombol expander** (bukan nested — fix a11y 2026-09-04), treatment **secondary outline**: `bg-white` + ikon lucide `MessageCircle` `h-4 w-4 stroke-[2.5]` + label "WA" `text-[8px]` dalam `text-neo-darkgreen`, `min-h-[44px] min-w-[44px]`, `rounded-r-[10px] border-l-2 border-black neo-press neo-press-sm`. Prop `onWhatsApp` MemberCard /pembayaran DIHAPUS (keputusan sadar: /pembayaran fokus Speed-Tap). Aturan warna baru: **`neo-green` solid dicadangkan eksklusif untuk status LUNAS + tombol aksi utama** — WA reminder pakai ink `neo-darkgreen` di atas putih agar hijau tidak bermakna ganda saat scan list. Deep-link via `waReminderUrl()` (`src/lib/format.ts`): `https://wa.me/<62xx>?text=<pesan>` — pesan template "Halo {nama}, pengingat iuran kas bulan {bulan} sebesar {nominal} ya. Terima kasih 🙏". Juga amendemen 2026-09-04: tombol aksi toast /pembayaran (`NEO_ACTION_STYLE`) kini putih/hitam (dulu inverted hitam/kuning).

### 5.8 Toast (sonner)
Styling neo-brutalist sesuai mockup in-app toast: `border-[3px] border-black bg-neo-yellow text-black font-extrabold text-xs rounded-xl shadow-neo-lg px-3.5 py-2.5` + ikon lucide `check` `stroke-[3]`. **Error toast = coral**: via `classNames` per-status di Toaster (`error: "bg-neo-coral"`, default/success kuning — amendemen 2026-09-02). Toast speed-tap = **undo toast `duration: 5000` + aksi "BATALKAN"** (lihat 5.10 Mitigasi Salah-Tap; auto-dismiss ~2.2s di mockup hanya demo, tidak dipakai). Toast sukses form pengeluaran: `toast.success` + `duration: 6000` (auto-dismiss 6 detik — amendemen 2026-09-04, dulu `duration: Infinity`) dengan aksi eksplisit "Input Lagi" yang tetap tersedia selama toast tampil.
...
3. Tombol "SIMPAN PENGELUARAN" `bg-neo-green` → toast sukses `toast.success` auto-dismiss 6 detik dengan aksi eksplisit "Input Lagi" (amendemen 2026-09-04). Validasi `jumlah > 0` di client (toast "Nominal pengeluaran harus > 0") + server (Zod).
...
> Amendemen 2026-09-04 (sesi lanjutan): (a) hint "Lupa PIN? Hubungi bendahara untuk reset." di LoginForm setelah grid PIN — recovery path untuk user pemula (label teks, bukan jargon teknis); (b) row /anggota: expander button dan WA anchor wajib **SIBLING** dalam `<li>` (interactive tidak boleh nested); (c) label kecil floor 9px — label "WA" `text-[8px]` pada tombol WA /anggota adalah **pengecualian sadar** (tombol 44px ikon-centric, bukan teks bacaan), jangan ditiru di elemen lain.
