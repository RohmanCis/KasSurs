...
**Status proyek:** T-01 s.d. T-37 + Modul R (UI Neo-Brutalism V2.2) + design-review batch fix + QA manual fix + code review 2-axis + fix batch + **UI polish & perf batch (2026-09-03, lihat bagian "UI Polish & Perf Batch")** + audit dokumentasi (39 temuan, commit `0e8d9ef`) SELESAI. Sisa T-38–T-39 (Deployment). Verifikasi terakhir (2026-09-03, HEAD `0e8d9ef`): tsc clean, vitest **161/161** (29 files), build OK + E2E 10/10 (4 spec) + curl live 5 security headers. **Batch presentational `4ff92e5`–`946863f` kini SUDAH di-rerun E2E (user, 2026-09-03 — P2 verification)**; re-test QA-MANUAL §4 alur V2.2 selesai (user-verified); klaim §6.2 "2 sheet Ringkasan+Transaksi" terverifikasi kode (`excel.ts:54-55`).
...
### UI Adjustments + WhatsApp Reminder (2026-09-04) — DIEKSEKUSI (user-directed, build clean, test TIDAK dijalankan per instruksi user)
Batch perubahan user-initiated, 5 file. Semua verifikasi via `npm run build` (clean ×3, tsc+lint lolos). **E2E/vitest belum dijalankan untuk batch ini** — jalankan saat verifikasi berikutnya.
- **`src/app/(admin)/pembayaran/page.tsx` — NEO_ACTION_STYLE tombol aksi toast:** inverted hitam/kuning (`#000000`/`#FEF08A`) → **putih/hitam** (`#FFFFFF`/`#000000`, border hitam tetap). Berlaku semua toast di /pembayaran (BATALKAN, dsb.). CATATAN: `pengeluaran/page.tsx:26-35` masih pakai style LAMA (hitam/kuning) — tidak diminta diubah, tidak disentuh.
- **`src/app/(admin)/laporan/page.tsx`:** +badge periode di header — render dari **state FilterBar** (bulan/tahun TERPILIH, bukan `now`) — `bg-neo-sky px-1.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wider`, sebelum subtitle. Header 5 halaman admin kini: dashboard=periode(subtitle), pengeluaran=periode(subtitle), laporan=periode(badge sky), pembayaran=TANPA periode (badge "1-Tap" saja), anggota=TANPA periode di header (periode+badge "BULAN INI" ada di konten bawah header dengan chevron). (FIX 2026-09-04 sesi lanjutan: badge kini dari state FilterBar — awalnya render dari `now`/bulan berjalan; kontradiksi header vs file yang diunduh saat filter diganti. Lihat bagian "Design Critique & Fix Batch".)
- **WhatsApp reminder (fitur baru — kini FR-24 di PRD V1.2):**
  - **`src/lib/format.ts`:** +`toWaNumber(noHp)` (strip non-digit; `0…`→`62…`; `62…` prefix preserved; fallback prepend `62`) + `waReminderUrl(nama, noHp, bulan, nominal)` → `https://wa.me/<62xx>?text=<encoded>` dengan pesan template "Halo {nama}, pengingat iuran kas bulan {bulan} sebesar {nominal} ya. Terima kasih 🙏". Nominal hardcoded "Rp 30.000" di call site (konstanta JUMLAH_DEFAULT pembayaran juga 30000 — konsisten).
  - **`src/app/(admin)/anggota/page.tsx`:** tombol WA — `<a target="_blank" rel="noopener noreferrer">` wa.me, kini **SIBLING tombol expander** di dalam `<li>` (bukan nested dalam button — fix a11y P1 sesi lanjutan 2026-09-04). Treatment **secondary outline** (redesign "Opsi A", proposa designer): `bg-white` + ikon `MessageCircle h-4 w-4 stroke-[2.5]` + label "WA" `text-[8px] font-extrabold uppercase leading-none`, keduanya `text-neo-darkgreen`; `min-h-[44px] min-w-[44px]`, `rounded-r-[10px] border-l-2 border-black neo-press neo-press-sm`. Render via helper `showWa` = `statusAktif && statusBayarBulanIni !== "LUNAS"`; `li` = `flex flex-wrap items-stretch`; radius expander kondisional `showWa ? "rounded-l-[10px]" : "rounded-[10px]"`. **Aturan warna baru:** solid `neo-green` dicadangkan untuk status LUNAS saja — WA pakai ink `neo-darkgreen` di atas putih agar hijau tidak bermakna ganda saat scan list.
  - **`src/components/dashboard/MemberCard.tsx`:** prop `onWhatsApp` DIHAPUS (sesi lanjutan 2026-09-04 — fix critique P1 nested interactive; sebelumnya +prop opsional + tombol WA `bg-neo-green` di baris bawah antara `jumlahTampil` dan pill, hanya saat Belum Bayar & handler ada). WA reminder kini EKSKLUSIF /anggota — keputusan sadar: /pembayaran fokus Speed-Tap, /anggota fokus manajemen + komunikasi.
  - **`src/app/(admin)/pembayaran/page.tsx`:** handler `bukaWa` + prop `onWhatsApp={bukaWa}` per MemberCard + import `waReminderUrl`/`NAMA_BULAN` DIHAPUS (sesi lanjutan 2026-09-04) — tidak ada lagi tombol WA di /pembayaran.
- **Implementasi noHp di MemberCard:** kartu menerima `member` object penuh (bukan prop noHp) — noHp tersedia via `member.noHp`, tidak dirender.

### Design Critique & Fix Batch (2026-09-04, sesi lanjutan) — DIEKSEKUSI
- Impeccable critique dual-agent (design review + detector scan clean) → skor Nielsen 30/40, 3 P1 + 2 P2. Snapshot: `.impeccable/critique/2026-09-04T00-00-55Z__src-app.md`.
- 5 fix dieksekusi: (1) nested interactive /anggota (WA `<a>` sibling expander `<button>`); (2) MemberCard `onWhatsApp` + pembayaran `bukaWa` dihapus — WA eksklusif /anggota; (3) badge periode /laporan render dari state FilterBar (dulu `now` — kontradiksi vs file unduhan); (4) toast sukses pengeluaran `toast.success` + `duration: 6000` (dulu `Infinity`); (5) badge "Bcrypt Hash" login dihapus + hint "Lupa PIN? Hubungi bendahara untuk reset.".
- Redesign row /anggota "Opsi A" (proposal designer, dieksekusi): WA slab demote jadi secondary outline (`bg-white` + ink `neo-darkgreen` + label "WA"), `items-stretch` di `li`, radius expander kondisional (`rounded-l-[10px]` saat WA ada, `rounded-[10px]` saat tidak). Rationale: solid green dicadangkan eksklusif status "✓ Lunas" (scan pattern), hierarki state > aksi.
- Caption hint long-press /pembayaran diperbesar: `text-[10px] text-slate-600` → `text-xs text-slate-700`.
- Verifikasi: `npm run build` clean per batch. **Vitest/E2E TIDAK dijalankan untuk batch WA maupun batch fix ini** — wajib saat verifikasi berikutnya.
- Keputusan mengikat baru: WA reminder hanya /anggota (jangan re-add ke /pembayaran tanpa arahan user); rapel tetap long-press + hint (pilihan user, keyboard path BELUM ditambahkan — kandidat follow-up); minor visual critique (toast style split /pengeluaran, kamu/Anda, Rp 30k vs Rp 30.000, text-[8px] pill, tabular-nums toast) di-CANCELLED scope — tersimpan di snapshot untuk nanti.
...
- **JetBrains Mono DIHAPUS dari project** (~25KB woff2, hanya dipakai 1x badge "Bcrypt Hash"): import + variable dihapus dari `src/app/layout.tsx`, badge `LoginForm.tsx:187` sempat `tabular-nums` (bukan `font-mono`), `tailwind.config.ts` fontFamily.mono → generic `ui-monospace/SFMono/Menlo/monospace`. **Lanjutan sesi 2026-09-04: badge "Bcrypt Hash" DIHAPUS SELURUHNYA** (critique P2 — jargon developer di layar pertama), ditambah hint "Lupa PIN? Hubungi bendahara untuk reset." (`text-[10px] font-bold text-slate-600`) setelah grid PIN. Perlu verify tsc + build + visual login.
...
- **2026-09-03:** Git init + push GitHub (RohmanCis/KasSurs, commit `db3e822`, 131 file). Mobile performance QA 3-lane (explorer+oracle+designer): 0 kritis, plan FASE 1-3 disusun & dicatat di bagian Keputusan — eksekusi ditunda menunggu user. JetBrains Mono dihapus dari project. Architecture review 4 kandidat deepening (laporan HTML temp dir) → **kandidat #3 (format.ts) + #4 (validation.ts) dieksekusi hari sama**: 17 file refactor, +2 modul lib, +23 unit test (2 file baru), vitest 138→**161/161**, tsc clean, kontrak literal utuh (integration test hijau). Detail di bagian Keputusan "Architecture Deepening". **API Handler Kit #1 dieksekusi hari sama** (blueprint oracle + fixer; 11/13 route migrasi, ±450 baris boilerplate dihapus, kontrak byte-identik; tsc clean, vitest 161/161, build OK). **Audit dokumen 3-lane** (AGENTS/HANDOFF/PRD/TECH-SPEC/DESIGN/TASKS/QA-MANUAL vs kode): 0 pelanggaran spec oleh kode, 0 dead-ref berbahaya; semua temuan stale-doc difix batch (28 edit + LOW 5 edit). **Kandidat #2 useApiData NO-GO** (ROI eval oracle — lihat bagian Keputusan). **Code review menyeluruh 2-axis** (Standards+Spec, diff `db3e822`→HEAD+working-tree, 2 oracle paralel): 0 hard violation/0 kritis; fix batch dieksekusi — commit `be3d299` (deepening #3/#4 + Kit #1 + todayISO WIB-safe) & `9568a66` (neo-press utility 13 site, alreadyPaid param wajib, hapus header no-op); verifikasi penuh tsc/vitest 161/build/E2E 10/curl. 3 issue laten lama DITUTUP keputusan user (lihat §3). **UI polish & perf batch** (user-initiated, 6 file: optimizePackageImports lucide-react, FilterBar chevron overlay, kategori pengeluaran grid 2-col + color dot — amendemen FR-09, BottomNav py-2 tanpa safe-area, kartu Belum Bayar font-mono, dynamic import drawer pembayaran; tsc clean; detail di bagian "UI Polish & Perf Batch").
- **2026-09-03 (lanjutan):** audit dokumentasi menyeluruh (**38+1 temuan**, commit `0e8d9ef`) + cleanup sync — QA-MANUAL/PRD/TECH-SPEC/DESIGN/TASKS/AGENTS/HANDOFF diupdate ke kondisi kode aktual; build bersih. **P2 verification (user, sesudahnya):** E2E re-run 10/10 untuk batch presentational `4ff92e5`–`946863f`, re-test QA-MANUAL §4 alur V2.2, klaim §6.2 excel 2 sheet diverifikasi kode — semua clean, tidak ada issue baru.
- **2026-09-04:** UI adjustments + **fitur baru WhatsApp reminder** (user-directed): NEO_ACTION_STYLE /pembayaran putih/hitam (dulu inverted hitam/kuning; /pengeluaran tetap lama), badge periode `bg-neo-sky` header /laporan, tombol WA di /anggota (row) + MemberCard /pembayaran (prop `onWhatsApp`), `toWaNumber`+`waReminderUrl` di format.ts. 5 file, build clean ×3. **Vitest/E2E TIDAK dijalankan (instruksi user)** — wajib dijalankan di sesi/verifikasi berikutnya. Detail di bagian "UI Adjustments + WhatsApp Reminder".
- **2026-09-04 (lanjutan):** Impeccable critique 30/40 (dual-agent) + 5 fix P1/P2 + redesign row anggota "Opsi A" + resize caption hint — detail di bagian "Design Critique & Fix Batch". Build clean; vitest/E2E belum (berlaku utk 2 batch).
