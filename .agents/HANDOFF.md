...
- **`src/components/layout/BottomNav.tsx`:** `h-16` fixed → `py-2` auto-height; **`pb-[env(safe-area-inset-bottom)]` DIHAPUS** — catatan: tanpa safe-area, di iPhone notch/home-indicator tab bisa nempel edge; diterima sebagai keputusan user (revert 1 baris kalau jadi masalah di iOS).
