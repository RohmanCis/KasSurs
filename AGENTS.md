...
cp .env.local .env   # Prisma CLI 5 baca .env, bukan .env.local (gotcha #1)
...
1. `.agents/1-PRD.md` — produk, 24 Functional Requirements (V1.2: FR-24 WA reminder baru 2026-09-04; V1.1: FR-06 Speed-Tap + FR-07/08/09/12/14 amendemen 2026-09-02; FR-09 + amendemen 2026-09-03 — kategori grid 2-col + color dot), NFR, out of scope.
...
3. `.agents/3-DESIGN.md` — **V2.2 Neo-Brutalism**: token warna flat `neo.*`, hard shadow, Bricolage Grotesque, resep komponen 5.1–5.10 (5.10 = alur Speed-Tap + mitigasi salah-tap FINAL), motion, aksesibilitas. Sinkron amendemen 2026-09-02 + 2026-09-03 (FilterBar chevron §5.6, kategori grid 2-col + dot §5.10, BottomNav §7). Ground truth visual: `.agents/kassurs_ui_neobrutalism_final.html`.
...
Status: SEMUA 39 task + Modul R SELESAI. **GO-LIVE PRODUCTION 2026-09-04: https://kas-surs.vercel.app** (T-38 Vercel + T-39 migrate/seed + cleanup DB + saldo awal base Rp3.710.300 + smoke 5/5 OK).

## Progress

Update terakhir: 2026-09-04 — **UI adjustments + fitur baru WhatsApp reminder** (user-directed, 5 file): NEO_ACTION_STYLE tombol aksi toast /pembayaran putih/hitam; badge periode `bg-neo-sky` header /laporan; tombol WA deep-link wa.me (`toWaNumber`+`waReminderUrl` di `src/lib/format.ts`) **eksklusif di row /anggota** (MemberCard /pembayaran dihapus sesi lanjutan — fix a11y nested interactive); sesi lanjutan: Impeccable critique 30/40 + 5 fix P1/P2 (nested interactive, badge /laporan dari state FilterBar, toast pengeluaran 6000ms, hapus badge Bcrypt + hint Lupa PIN) + redesign row anggota (WA secondary outline bg-white/darkgreen) + caption hint text-xs + **sinkron NEO_ACTION_STYLE /pengeluaran putih/hitam** + **E2E flaky fix** (timeout 60s + globalSetup purge data "E2E ..." — `tests/e2e/global-setup.ts` baru; root cause: DB bloat 192+ member sisa E2E, bukan bug aplikasi). Build clean; **E2E TERVERIFIKASI 10/10 (2026-09-04, melunasi utang verifikasi E2E batch 2026-09-04); vitest 161/161 (29 files) TERVERIFIKASI 2026-09-04 — utang verifikasi batch 2026-09-04 LUNAS**. Detail: HANDOFF.md "UI Adjustments + WhatsApp Reminder" + "Design Critique & Fix Batch" + "E2E flaky fix". Sebelumnya (2026-09-03): Mobile perf QA + FASE 1-3 eksekusi + deepening #3/#4 + API Handler Kit #1 + audit dokumen + **code review menyeluruh 2-axis (Standards+Spec, diff db3e822→HEAD) + fix batch** + **UI polish & perf batch** (optimizePackageImports lucide-react, FilterBar chevron, kategori grid 2-col + dot — amendemen FR-09, BottomNav py-2, dynamic import drawer) + **restore safe-area-inset-bottom di BottomNav + chip kategori 36px sebagai keputusan sadar user** (`946863f`). Detail state, keputusan & history: `.agents/HANDOFF.md`. Audit dokumentasi menyeluruh + cleanup sync 39 temuan (semua doc sync ke kode aktual).

- **Done:** T-01 s.d. T-37 + Modul R (UI V2.2) + design review fix + QA manual fix + **deepening #3/#4 (2026-09-03)**: `src/lib/format.ts` (satu sumber NAMA_BULAN/formatRupiah/todayISO WIB-safe — 11 file konsumen) + `src/lib/validation.ts` (dateOnly/minimalSatuField/parseBulanTahunQuery — 6 route handler); 23 unit test baru + **FASE 1/2/3 + error/not-found (2026-09-03)** + **API Handler Kit #1 (2026-09-03)**: `src/lib/api/{respond,session}.ts` + `src/lib/dto/{payment,expense,member,category}.ts` — 11/13 route migrasi, ±450 baris boilerplate dihapus, kontrak byte-identik. Verifikasi terakhir: tsc clean, vitest **161/161** (29 files, 2026-09-04). E2E 10/10 (4 spec, terakhir 2026-09-04 dengan globalSetup purge). JetBrains Mono dihapus (badge login pakai `tabular-nums` + generic mono stack).
- **Next:** Tidak ada open task — proyek go-live 2026-09-04 (https://kas-surs.vercel.app). Kandidat lanjutan menunggu arahan user: ganti PIN admin prod (`000000` → kuat), tambah anggota asli, favicon 404, keyboard path rapel. Kandidat #2 useApiData NO-GO (ROI eval 2026-09-03 — lihat HANDOFF.md); kandidat arsitektur deepening #1–#4 semua selesai/ditutup. Plan lengkap: HANDOFF.md §4.
...
4. **Playwright chromium sudah ter-install (T-37)** — e2e spawn dev server sendiri port 3100 dengan `DATABASE_URL` override ke test DB 5433 (`reuseExistingServer: false` — jangan reuse server 3100 lain yang baca dev DB). `expect.timeout` 15s karena cold-compile route Next dev-mode; per-test `timeout: 60_000` (fix flaky 2026-09-04 — test drawer terberat bisa >30s saat roster besar).
...
9. **Test paralel tabrakan noHp unique — SKEMA BARU: salt per-file** (pengganti alokasi digit lama yang terbukti bocor). Setiap file test integration menambahkan 1 char salt unik ke `uniq`: `const uniq = String(Date.now()) + "x"` — terpakai: a=members, b=payments, c=payments-patch, d=members-patch, e=members-deactivate, f=expenses, g=login, h=audit, i=expenses-patch, j=dashboard, k=reports, l=lockout-e2e, m=race-payments, n=race-categories, o=expenses-p2003, p=expenses-edge, q=summary-empty-db, r=summary-race, s=report-race, t=members-reactivate. File baru pakai huruf berikutnya. Pengecualian pola: `login.test.ts` (salt `g`) & `audit.test.ts` (salt `h`) pakai suffix inline di template noHp (`` `08${String(Date.now()).slice(-10)}g` ``), tanpa variabel `uniq`. Salt membuat noHp/nama antar file mustahil identik walau `Date.now()` sama — semua `slice(-N)` tetap bekerja. (Suite kini serial + DB terisolasi, jadi salt murni kebersihan data — tetap wajib.)
...
12. **`Date.parse` TIDAK menolak tanggal kalender invalid** — `"2026-02-30"` lolos regex + NaN check (V8 silent rollover → Mar 2). Validasi date-only WAJIB pakai roundtrip: `new Date(s).toISOString().slice(0,10) === s` (temuan oracle T-24; kini terpusat di `src/lib/validation.ts` `dateOnly()` — jangan duplikat ulang di route).
